...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/* RTOS object handles (defined & created in freertos.c / MX_FREERTOS_Init) */
extern osSemaphoreId_t startSem;
extern osSemaphoreId_t finishSem;
extern osSemaphoreId_t wordCountSem;
...
int main(void)
{
    /* 1. HAL library and system clock init */
    HAL_Init();
    SystemClock_Config();
    ...
    /* 3. Initialize the RTOS kernel */
    osKernelInitialize();

    /* 4. Create semaphores, mutex and tasks */
    MX_FREERTOS_Init();
